...
if(isset($_POST['type']) && $_POST['type'] == 'student')
{
    ...
    $std_enroll = (!empty($_POST['enrollment_no']) && trim($_POST['enrollment_no']) !== '') ? $_POST['enrollment_no'] : date('Y', strtotime($doa)).date('dm', strtotime($dob)).sprintf('%06d',$std_enroll);
    $usermeta = [];
    foreach ($_FILES['documention']['name'] as $key => $value) {
        if (!empty($_FILES['documention']['name'][$key])) {
    ...
    {    
        ...
        if(mysqli_num_rows($check_query) > 0)
        {
            ...
            $parent = get_user_data($parent_id,false);
                $parent_account_name = mysqli_real_escape_string($db_conn, $father_name);
                $parent_account_email = mysqli_real_escape_string($db_conn, $father_mobile);
                mysqli_query($db_conn, "UPDATE `accounts` SET `name` = '$parent_account_name', `email` = '$parent_account_email' WHERE `id` = '$parent_id' AND `type` = 'parent'") or die(mysqli_error($db_conn));
            ...
            foreach ($parent_meta as $key => $value) {
                ...
                $check_parent_meta = mysqli_query($db_conn, "SELECT * FROM `usermeta` WHERE `user_id` = $parent_id AND `meta_key` = '$key'");
                if (mysqli_num_rows($check_parent_meta) > 0) {
                    mysqli_query($db_conn, "UPDATE `usermeta` SET `meta_value` = '$value' WHERE `user_id` = $parent_id AND `meta_key` = '$key'") or die(mysqli_error($db_conn));
                } else {
                    mysqli_query($db_conn, "INSERT INTO usermeta (`user_id`,`meta_key`,`meta_value`) VALUES ('$parent_id','$key','$value')") or die(mysqli_error($db_conn));
                }
            }
        }
        else
